...
#if MQTT_CHECK == 1
		MQTT_Sub((char *)SIM800.mqttCheckTopic);
		mqtt_check = HAL_GetTick();
		MQTT_PubUint32((char *)SIM800.mqttCheckTopic, mqtt_check);
#endif
	}
}
...
void MQTT_Receive(unsigned char *buf)
{
	...
	MQTTDeserialize_publish(&SIM800.mqttReceive.dup, &SIM800.mqttReceive.qos, &SIM800.mqttReceive.retained, &SIM800.mqttReceive.msgId, &receivedTopic, &payload,
							&SIM800.mqttReceive.payloadLen, buf, sizeof(buf));
...
#if MQTT_CHECK == 1
	if (strcmp(SIM800.mqttCheckTopic, (char *)SIM800.mqttReceive.topic) == 0 && atol((char *)SIM800.mqttReceive.payload) == mqtt_check)
	{
		mqtt_connected = 1;
	}
#endif
...
{
	SIM800.mqttServer.connect = mqtt_connected;
	mqtt_check = HAL_GetTick();
	MQTT_PubUint32((char *)SIM800.mqttCheckTopic, mqtt_check);
	mqtt_connected = 0;
}
